...
The term *refactoring* means restructuring your code to improve your code internally while keeping its external functionality. In other words, it means cleaning and modularizing your code after you've got it working.
...
Let's consider a scenario where we would like to calculate our total cost after including GST (say 15%), one might implement it this way:
```{python.reticulate=FALSE}
total_price = 0
...
```
In this scenario, we've implemented a function which returns the result of a division operation. However, we need to handle cases where the denominator could be equal to zero and the way we handle is with the `ZeroDivisionError` exception.
```{python.reticulate=FALSE}
...
Here is an example, say we implemented a function that calculates the Euclidean distance between two vectors. The code file is `./utils/eucl_distance.py`.
```{python.reticulate=FALSE}
import numpy as np
...
In order to test it, we create our test file `./utils/test_eucl_distance.py` as follows:
```{python.reticulate=FALSE}
import unittest
import numpy as np
...
```
Then we just run `pytest` in the relevant folder.
